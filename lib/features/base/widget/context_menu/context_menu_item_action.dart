...
abstract class ContextMenuItemAction<T> with EquatableMixin {
  ...
  ContextMenuItemAction(this.action, {this.category = -1});
...
abstract class ContextMenuItemActionRequiredIcon<T>
  ...
  ContextMenuItemActionRequiredIcon(super.action, {super.category});
}
...
abstract class ContextMenuItemActionRequiredFull<T>
    extends ContextMenuItemAction<T>
    with OptionalIcon, OptionalSelectedIcon<T> {
  final T selectedAction;

  ContextMenuItemActionRequiredFull(
    super.action,
    this.selectedAction, {
    super.category,
  });
