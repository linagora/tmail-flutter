  ...
  Stream<Either<Failure, Success>> execute(
    ...
    try {
      ...
      if (publicAssets.isEmpty) {
        yield Left(AddIdentityToPublicAssetsFailureState());
      } else {
        final publicAssetsWithCurrentIdentity = publicAssets
          .map((publicAsset) => PublicAsset(
            id: publicAsset.id,
            publicURI: publicAsset.publicURI,
            size: publicAsset.size,
            contentType: publicAsset.contentType,
            blobId: publicAsset.blobId,
            identityIds: publicAsset.identityIds?..addAll({identityId: true}),
          ))
          .toList();
