...
class ComposerController extends BaseController
  ...
  void openNewModal() {
    if (PlatformInfo.isWeb) {
      ...
      _buildIframeUrl(sessionId).then((url) {
        if (url == null || _activeIframeSessionId != sessionId) return;
...
Future<String?> _buildIframeUrl(String sessionId) async {
    const token = 'eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJhbGljZS5jb3p5LmxvY2FsaG9zdDo4MDgwIiwic3ViIjoidGVtcGxhdGUiLCJhdWQiOlsiYXBwIl0sImlhdCI6MTc4MDMwMTg4Nywic2Vzc2lvbl9pZCI6IjRiZTA0ZjM3NGVhY2FmNWNmNTgzOWVlZWI4MWEzM2YwIn0.n0kaYHtiyNKGI7h2yjkcTLXtfbwDrVCGnntW6hgPHkxmTCdJEqPvoL2dQlu3VxFj-cAZYlt-zIoAv9DlqVDPjA';

    try {
      final dio = Dio();

      final response = await dio.post(
        'http://alice.cozy.localhost:8080/intents',
        data: {
          'data': {
      ...
      final services = response.data['data']?['attributes']?['services'];
      if (services is List && services.isNotEmpty && services[0] is Map) {
        return services[0]['href'] as String?;
      }
      return null;
    } catch (e) {
      debugPrint('_buildIframeUrl error: $e');
      return null;
    }
  }
  ...
  void _processPostMessageData(Map<dynamic, dynamic> messageData, String sessionId) {
    ...
    // Check if message type contains "ready" and respond with empty object
    if (messageData.containsKey('type') && 
        messageData['type']?.toString().toLowerCase().contains('ready') == true) {
      debugPrint('✅ Ready message received, sending empty object response');
