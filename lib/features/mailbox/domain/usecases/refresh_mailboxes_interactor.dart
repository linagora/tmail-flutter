...
class RefreshMailboxesInteractor {
  final MailboxRepository _mailboxRepository;

  RefreshMailboxesInteractor(this._mailboxRepository);

  Stream<Either<Failure, Success>> execute(AccountId accountId, jmapState.State currentState) async* {
    try {
      yield Right<Failure, Success>(LoadingState());

      final streamListMailbox = _mailboxRepository.refresh(accountId, currentState);

      final streamResult = streamListMailbox.asyncExpand((response) async* {
        final tupleList = response.mailboxes?.splitMailboxList((mailbox) => mailbox.hasRole()) ?? Tuple2([], []);
        yield Right<Failure, Success>(GetAllMailboxSuccess(
          defaultMailboxList: tupleList.value1,
          folderMailboxList: tupleList.value2,
          currentMailboxState: response.state));
      });
