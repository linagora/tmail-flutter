...
class DeleteMultipleMailboxInteractor {
  ...
  Stream<Either<Failure, Success>> execute(AccountId accountId, List<MailboxId> mailboxIds) async* {
    try {
      final result = await mailboxRepository.deleteMultipleMailbox(accountId, mailboxIds);
      if (result) {
