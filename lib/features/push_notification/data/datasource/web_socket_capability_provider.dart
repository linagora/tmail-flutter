...
///
/// ## JMAP WebSocket Capability
///
/// Per RFC 8887, JMAP advertises WebSocket support at the **session level**
/// (not account level) via the `urn:ietf:params:jmap:websocket` capability.
///
/// The capability object contains:
/// - `url`: The WebSocket endpoint URL
/// - `supportsPush`: Whether push notifications are supported (optional)
///
/// ## Server Differences
///
/// Different servers handle the `supportsPush` property differently:
///
/// | Server    | supportsPush value | Supports Push? |
/// |-----------|-------------------|----------------|
/// | James     | `true`            | Yes            |
/// | Stalwart  | `null` (omitted)  | Yes            |
/// | Others    | `false`           | No             |
///
/// This class treats `null` as "supports push" since the capability exists.
/// Only an explicit `false` disables push support.
///
/// ## Usage
///
/// ```dart
/// final provider = WebSocketCapabilityProvider();
///
/// // Check if WebSocket is available
/// if (provider.hasCapability(session)) {
///   // Validate and get URI
