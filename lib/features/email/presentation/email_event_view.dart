...
import 'package:tmail_ui_user/features/email/presentation/widgets/app_bar_mail_event_widget_builder.dart';
...
import '../../base/widget/material_text_button.dart';

class EmailEventView extends GetWidget<SingleEmailController> {

  final responsiveUtils = Get.find<ResponsiveUtils>();
  final imagePaths = Get.find<ImagePaths>();

  EmailEventView({Key? key}) : super(key: key);
  ...
  Widget _buildAppBar(BuildContext context, PresentationEmail presentationEmail) {
    return Obx(() => AppBarMailEventWidgetBuilder(
      presentationEmail,
      ...
      builder: (context, constraints) {
        return Column(crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            ...
            _buildLoadingView(),
            _buildTopOfEmail(context, email),
            if (BuildUtils.isWeb)
  ...
  Widget _buildTopOfEmail(BuildContext context, PresentationEmail email) {
   return Obx(() {
     final attachments = controller.attachments.listAttachmentsDisplayedOutSide;
     return attachments.isNotEmpty
         ? _buildTopOfEmailBody(context,email)
         : const SizedBox.shrink();
   });
  }

  Widget _buildTopOfEmailBody(BuildContext context, PresentationEmail email) {
    return Column(
      children: [
        Row(
          children: [
              Container(
                ...
                image: DecorationImage(
                    image: AssetImage("images/ic_calender_image.png"), fit: BoxFit.cover)),
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      Text(email.receivedAt!.value.month.toString(),
                      style: const TextStyle(
                      color: Colors.black,
                      fontSize: 20),),
                      Spacer(),
                       Text(email.receivedAt!.value.day.toString(),
                      style: const TextStyle(
                      color: Colors.black,
                      fontSize: 20),),
                       Spacer(),
              ...
              Expanded(
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children:  [
                      _tilteOfEmail("Daily Meeting Linagora",null),
                      _viewOnColender("View on Calendar", null),
                      _lineOfEmailInfo('When',null ,'Fri Feb 24, 2023 3pm - 4pm (ICT)',null),
                      _lineOfEmailInfo('Where',null ,'Fri Feb 24, 2023 3pm - 4pm (ICT)',null),
                      _lineOfEmailInfo('Who',null ,'test@mail3.com test@mail3.com',null),
    ...
    Container(height: 40,
      color: AppColor.toastSuccessBackgroundColor,
      child:Row(
        children: const [
          Expanded(child:  Padding(
  ...
  Widget _tilteOfEmail(String label1 ,Function? onTap){
  ...
  Widget _lineOfEmailInfo(String label1 ,Function? onTap1, String label2,Function? onTap2){
    return Row(
      children: [
        _emailInfo(label1,onTap1), _emailInfo(label2,onTap2)
      ],
    );
  }
...
Widget _viewOnColender (String label , Function? onTap , ) {
    return MaterialTextButton(
              label: label,
               onTap: () => onTap,
              borderRadius: 8,
              padding: const EdgeInsets.all(3),
              labelSize: 20,
              labelColor: AppColor.colorTextButton,
              labelWeight: FontWeight.w400,
              overflow: CommonTextStyle.defaultTextOverFlow,
              softWrap: CommonTextStyle.defaultSoftWrap, 
    );
  } 
  Widget _emailInfo (String label , Function? onTap) {
    return MaterialTextButton(
              label: label,
               onTap: () => onTap,
              borderRadius: 8,
