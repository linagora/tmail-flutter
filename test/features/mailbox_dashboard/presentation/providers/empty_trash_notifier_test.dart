...
void main() {
  ...
  late EmptyTrashNotifier notifier;
  ...
  setUp(() {
    ...
    notifier = container.read(emptyTrashProvider.notifier);
  });
  ...
  List<EmptyTrashState> listenStates() {
    final states = <EmptyTrashState>[];
    container.listen(
      emptyTrashProvider,
  ...
  EmptyTrashSuccess readSuccess() =>
      container.read(emptyTrashProvider) as EmptyTrashSuccess;

  group('EmptyTrashNotifier', () {
    group('initial state', () {
      test('starts as EmptyTrashIdle', () {
        expect(container.read(emptyTrashProvider), isA<EmptyTrashIdle>());
      });
    });
    ...
    group('execute — JMAP clear path (useJmapClear = true)', () {
      test('emits Loading then Success when clear succeeds with no subfolders',
        ...
        expect(states, [
          isA<EmptyTrashLoading>(),
          isA<EmptyTrashSuccess>(),
        ]);

        final success = states.last as EmptyTrashSuccess;
      ...
      test('emits Failure when clear interactor throws', () async {
        ...
        when(mockClearInteractor.execute(any, any, any, any))
            .thenAnswer((_) => Stream.error(error));

        await notifier.execute(session, accountId, trashMailbox, [], true);

        final state = container.read(emptyTrashProvider);
        expect(state, isA<EmptyTrashFailure>());
        expect((state as EmptyTrashFailure).exception, error);
      });
      ...
      test('emits Failure when interactor is not registered', () async {
        ...
        expect(container.read(emptyTrashProvider), isA<EmptyTrashFailure>());
      });
    });
