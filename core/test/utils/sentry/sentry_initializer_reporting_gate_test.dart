...
class _MockSentryTransaction extends Mock implements SentryTransaction {}
...
void main() {
  ...
  group('SentryInitializer autonomous senders', () {
    test('keeps native monitoring enabled while the SDK is running', () {
      final options = SentryFlutterOptions()
  ...
  group('SentryInitializer secondary reporting gates', () {
    ...
    test('drops transactions while reporting is off', () {
      sentryManager.setSentryReportingConsent(false);

      expect(
        SentryInitializer.beforeSendTransactionHandler(
          _MockSentryTransaction(),
          Hint(),
        ),
        isNull,
      );
    ...
    test('keeps transactions while reporting is allowed', () {
      final transaction = _MockSentryTransaction();

      expect(
        SentryInitializer.beforeSendTransactionHandler(transaction, Hint()),
        same(transaction),
      );
    });
  });
