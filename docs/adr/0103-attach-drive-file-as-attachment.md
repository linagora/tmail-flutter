...
- Stream receive-timeout is uncapped (`Duration.zero`) on the Dio-backed download legs (IO, web-buffered); a stalled server relies on the user's manual cancel action. `receiveTimeout`, when set, actually guards both time-to-first-byte/headers *and* the gap between subsequent byte events (verified against Dio 5.2.0 and current 5.9.0 source) — it is not a to-first-byte-only timeout. `Duration.zero` disables it entirely for both cases, which is why a stalled mid-transfer server is not caught automatically and manual cancel is the only recourse today; an inactivity watchdog is tracked as accepted follow-up work, not a blocker for this ADR.
