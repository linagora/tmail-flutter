...
Updated 2026-07-06 — a single `SearchFilterNotifier` is the sole SSOT for every surface; the `SearchFilterDraftNotifier` staging model is removed (see Decision §1).
...
suggestion chips + advanced form + result chips                               SearchEmailNotifier
        ...
        └───────────────────────────────────► SearchFilterNotifier          │ execute(intent):   │
                        ...
                        (Obx web widgets)                                AsyncValue<SearchEmailResult>
