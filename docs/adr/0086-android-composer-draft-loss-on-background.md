...
Add an `AppLifecycleListener` to `ComposerController` (mobile only). On `onInactive`, extract the editor HTML and all composer fields (recipients, subject, identity, attachments metadata), then persist them to a new Hive box (`composerMobileCache`) keyed by `composerId`.
...
This is **fire-and-forget** — failure is logged but not surfaced to the user. The draft will appear in the Drafts folder on success.

#### Layer 3 — Recovery at `AppLifecycleState.resumed` / `onReady` (RC1)

On `ComposerController.onReady()` (mobile only), check whether a non-clean-close cache entry exists for the current `composerId`. If found, inject the cached HTML into the editor and restore all composer fields. Delete the cache entry after successful restoration.
...
ComposerController (GetX)
    ...
    │                   .saveLocally(composerId, content, recipients, subject, ...)
    │
    ...
    └── _closeComposerAction() [mark clean]
            └── appProviderContainer.read(composerAutoSaveProvider.notifier)
                        .markCleanClose(composerId)
```

```
ComposerAutoSaveNotifier (Riverpod StateNotifier)
    │
    ├── saveLocally(composerId, SavedComposerSnapshot)
    │       └── ComposerMobileCacheClient.insertItem(key, json)
    │
    ├── restore(composerId) → SavedComposerSnapshot?
...
- Local cache stores HTML content including potentially sensitive email body text. This is consistent with the existing web `ComposerCache` behaviour and is scoped to the device's Hive storage (no additional network exposure).
